...
CSClient.prototype.buildBackupData = function (credentials, backupRequest) {
    ...
    if (!this._verifyBackupRequestSignature(backupRequest, credentials.requestPubKey))
        throw new Error('Invalid backupRequest Signature');
    var decData = JSON.parse(walletUtils.decryptMessage(backupRequest.partialData, credentials.sharedEncryptingKey));
    ...
    if (decData.req_data.reqId !== backupRequest.reqId ||
        decData.req_data.reqCopayer !== backupRequest.copayerId ||
    ...
    //var pubKeys = [];
    //for (i=0; i<3; i++) {
    //    if (credentials.publicKeyRing[i].xPubKey !== credentials.xPubKey)
    //        pubKeys.push(credentials.publicKeyRing[i].xPubKey);
    //}
    var pubKeys = credentials.publicKeyRing.map(function (item) {
        return item.xPubKey;
    }).filter(function (xpk) {
    ...
    var serverPubKey;
    if (this.bwutils.verifyMessage(msg, decData.data_signature, pubKeys[0]))
        serverPubKey = pubKeys[1];
    else if (this.bwutils.verifyMessage(msg, decData.data_signature, pubKeys[1]))
        serverPubKey = pubKeys[0];
    else
...
CSClient.prototype._verifyBackupRequestSignature = function (backupRequest, pubkey) {
    var params = this._createBackupRequestParams(backupRequest.walletId, backupRequest.reqCopayer, backupRequest.reqTimestamp);
    var message = [params.method.toLowerCase(), params.url, JSON.stringify(params.data || {})].join('|');
    return this.bwutils.verifyMessage(message, backupRequest.signature, pubkey);
};
