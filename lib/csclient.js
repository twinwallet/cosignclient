...
CSClient.prototype._prepareBackupPartialData = function (credentials, backupPassword, backupRequest) {
    ...
    var saltBits = self.sjcl.codec.hex.toBits(credentials.xPrivKey);
    var hX = self.sjcl.misc.pbkdf2(backupPassword, saltBits, 1000, 128);
    var data = {
        req_data: backupRequest,
        encryptedKey: self.sjcl.codec.base64.fromBits(encXPK),
    ...
    this.httpRequest(getParams)
        .then(function successCB(response) {
            ...
            if (!_.isPlainObject(data)) {
                ...
                //TODO check data fields and validate data.partialData if it is present
                cb(null, data)
            }
        }, function errorCB(response) {
            cb(new Error(response.data));
 ...
 * @return
 */
CSClient.prototype.completeBackupData = function (credentials, backupRequest) {
    return {
